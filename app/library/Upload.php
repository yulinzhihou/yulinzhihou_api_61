...
class Upload
{
    ...
    public function upload($saveName = null, int $adminId = 0, int $userId = 0): array
    {
        ...
        $attachmentInfo = Attachment::where([
            ['sha1', '=', $params['sha1']],
            ['topic', '=', $params['topic']],
        ...
        if (!$attachmentInfo) {
            // 查询附件不存在，直接新增
            $attachmentInfo = $this->doSaveToAttachment($params,$saveName);
        } else {
            // 查询附件存在，判断文件是否存在，如果文件不存在，则重新新增记录。否则直接返回
            $destDir   = root_path() . 'public' . str_replace('/', DIRECTORY_SEPARATOR, $attachmentInfo['url']);
            if (!is_file($destDir)) {
                // 表示文件不存在，直接执行新增操作
                $attachmentInfo = $this->doSaveToAttachment($params,$saveName);
            }
     ...
     * @param $params
     * @param $saveName
     * @return Attachment|array|mixed|Model|nullAttachment|array|mixed|Model|null
     * @throws DataNotFoundException
     * @throws DbException
     * @throws ModelNotFoundException
     */
    private function doSaveToAttachment($params,$saveName)
    {
        $attachment = new Attachment();
        $attachment->data(array_filter($params));
        $res = $attachment->save();
        $destDir = root_path() . 'public' . str_replace('/', DIRECTORY_SEPARATOR, $params['url']);
        if ($res && !is_file($destDir)) {
            $this->move($saveName);
        }
